...
For a runnable SDK-to-CLI handoff using an installed package, see the
[adopter workflow](ADOPTER_WORKFLOW.md). It exports exact source bytes and separate
caller-owned anchors, gates native policy evaluation on source readiness, and
replays the accepted result in a fresh process.

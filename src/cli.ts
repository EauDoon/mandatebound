...
import { inventoryCaseEvidence, compareCaseCoverage, compareCaseEnvelopes } from "./operator-review.js";
...
export async function runCli(
  ...
  try {
    ...
    switch (args.command) {
      ...
      case "operator": {
        const invocation = requireSubcommandInput(args, ["triage", "checklist", "batch", "compare", "audit", "inventory", "queue", "coverage-diff", "envelope-diff"]);
        ...
        if (["compare", "coverage-diff", "envelope-diff"].includes(invocation.action)) {
          ...
          const compare = invocation.action === "coverage-diff" ? compareCaseCoverage
            : invocation.action === "envelope-diff" ? compareCaseEnvelopes : compareCaseAssessments;

    ...
    switch (args.command) {
      ...
      case "operator": {
        ...
        if (["batch", "queue", "bottlenecks", "findings"].includes(invocation.action)) {
          ...
          const result = invocation.action === "bottlenecks" ? findBatchCollectionBottlenecks(cases)
            : invocation.action === "findings" ? summarizeBatchFindings(cases) : assessCases(cases);
          writeJson(stdout, { ok: result.valid, result });
          return result.valid ? CLI_EXIT.SUCCESS : CLI_EXIT.INVALID;
        }

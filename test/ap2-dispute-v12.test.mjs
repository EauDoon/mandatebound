...
test("delegated AP2 terminal payload requires explicit expiry", () => {
  const fixture = makeFixture();
  const token = createMandate({
    iss: "https://trusted-surface.example",
    vct: "mandate.checkout.1",
    iat: 1_770_000_000,
    checkout_jwt: fixture.checkoutJwt,
  ...
  assert.equal(codes(report).has("AP2_EXPIRY_MISSING"), true);
});

test("AP2 chain verifier bounds malformed key snapshots", () => {
  const fixture = makeFixture();
  let report;
  assert.doesNotThrow(() => {
    report = verifyAp2MandateChain({
      ...fixture.verificationPlan.checkoutMandate,
  ...
  assert.equal(codes(report).has("INTEROP_KEY_SNAPSHOT_INVALID"), true);
});
